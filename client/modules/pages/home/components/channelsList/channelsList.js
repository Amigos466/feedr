...
function controlRssList(url, action, fresh, add) {
...
function channelsList(props) {
    ...
    return (
        ...
        placeholder="Enter channel url" />
            <span className="input-group-btn"><button onClick={ () => confirmerControl('add_channel', 'add')} className="btn btn-default" type="button"> Add channel! </button></span>
          </div>
          <div className={styles.scroller}>
            <ul className="list-group">
              { Object.keys(props.saved_list).map((e) => {
            return (<li onClick={() => props.updateCurrentChannel(props.saved_list[e])} key={ e } className="list-group-item justify-content-between">
